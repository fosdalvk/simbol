...
function ::unit:staticTestFunctions() {
    ...
    local modulecaps="$(::unit:modulecaps "${module}")"
...
function ::unit:modulecaps() {
    ...
    local mc
    IFS='.' read -a mc <<< "${module}"
    printf "%s" "${mc[@]^}"
}
#. }=-
#. ::unit:testFunction() -={
function ::unit:testFunction() {
    core:raise_bad_fn_call $# 2 4
...
function ::unit:writeDynamicTestFunctions() {
    ...
    local utf="$(::unit:testFunction ${profile} ${module} ${fn} ${context})"

    local -a dynamic_tests=( )
    local testVarDatum
    local -i i=0
    while read testVarDatum; do
        ((i++))
        local ffn="$(::unit:dynamicTestFunction ${module} ${fn} ${context})"
...
!SCRIPT
    ...
    if [ ! -e "${csv_in}" ]; then
        ...
        awk -F\| "BEGIN{b=0};NF==2&&\$1~/^#${profile}$/&&\$2~/^${module?}$/{b=1};END{exit(b)}" "${csv_in}"
    ...
    local utf="$(::unit:testFunction ${profile} ${module})"
    cat <<!SCRIPT >> ${script_out}
...
function ::unit:generate_tests() {
    ...
    local module="$1"
    cpfi "Verifying %{@module:${module}} eligibility...";
    ...
    for profile in ${!profiles[@]}; do
        ...
        -=[
            if [ -e "${modulepath}" ]; then
                cpfi "Generating %{@profile:%s}.%{@module:%s} header..."\
                    "${profile}" "${module}"
                ::unit:generate_header ${profile} ${module} ${modulepath}
                theme HAS_AUTOED $?

                cpfi "Generating %{@profile:%s}.%{@module:%s} body...\n"\
                    "${profile}" "${module}"
                -=[
                    ::unit:generate_body ${profile} ${module} ${modulepath}
                    e=$?
                ]=-

                cpfi "Generating %{@profile:%s}.%{@module:%s} footer..."\
                    "${profile}" "${module}"
...
function ::unit:execute_tests() {
    cpfi "Executing tests for %{@module:${module}}...\n"
    -=[
...
function unit:test() {
    ...
    local allowed=0
    local module
    local -a modules=( $(core:modules "$@") )

    if [ -e "${SHUNIT2?}" ]; then
        e=${CODE_SUCCESS?}
        for module in "${modules[@]}"; do
            ::unit:generate_tests "${module}"
            ::unit:execute_tests "${module}"
            ((e|=$?))
        done
