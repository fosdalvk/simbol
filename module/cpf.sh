...
function -=[() { local -i e=$?;  ((CPF_INDENT++)); return $e; }
function ]=-() { local -i e=$?; ((CPF_INDENT--)); return $e; }
function cpf:indent() {
...
function cpf:printf() {
    [ ${g_DEBUG?} -eq ${FALSE?} ] || set +x
    if [ $(core:len SIMBOL_ESCAPE_SEQUENCES) -eq 0 ]; then
        [ -t 1 ]; cpf:initialize $((~$?+2))
    fi
        ...
        case "${char}/${context}/${stacksize}" in
            ...
            */%/*)
            ...
            :/@/[1-9]*)
                ...
                [ $? -eq 0 ] || core:raise EXCEPTION_BAD_FN_CALL "No such theme \`${buffer}'"
